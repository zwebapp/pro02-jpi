        ...
        @if (count($orders) > 0)
          @foreach ($orders as $order)
            <tr class="{{ strtolower($order->status->value) }}"> 
              ...
              <td> <a href="{{ action('Clients@orderInfo', $order->id) }}" data-toggle="modal" data-target="#modalBox">{{ $order->created_at }} </td>
              <td> {{ isset($order->agent_id) ? json_decode($order->agent->information)->full_name : '' }} </td>
              <td>{{ $order->approved_at ?: '' }} </td>
              <td> 
              	@if ($order->status->id == 1 )
              		<a class="btn btn-mini btn-danger" href="{{ action('Clients@cancelOrder', $order->id) }}" title="Cancel Order"><i class="icon-white icon-remove"></i> </a> 
              	@endif
              </td>
            </tr>
          @endforeach
...
<!-- Modal -->
<div id="modalBox" class="modal hide fade profile-orfers" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
